...
/*   Updated: 2017/04/08 15:15:49 by djean            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		bool set_context)
{
	...
	if (node->command.words.len == 0)
		return (exec_simple_command_binary(node->command, input, set_context));
	...
	if (is_builtin(input->str + token->start, token->len))
		...
		ret = exec_simple_command_binary(node->command, input, set_context);
	return (ret);
}
